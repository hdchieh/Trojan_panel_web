...
if test -s /etc/nginx/nginx.conf; then
  ...
  sed -i "s/localhost/$your_domain/;" /etc/nginx/nginx.conf
	...
	green "================================="
	sleep 2s
  sed -i '/ExecStart/s/trojan web -p 2333/trojan web/g' /etc/systemd/system/trojan-web.service
  sed -i '/ExecStart/s/trojan web/trojan web -p 2333/g' /etc/systemd/system/trojan-web.service
